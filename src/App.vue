  ...
  setup() {
    ...
    watchEffect(() => {
      ...
      if (userStore.getState().access_token) {
        postToServer({ url: 'http://smart.agromelio.ru:8080/api/auth/users/me', request: 'get' })
          .then((response) => {
            console.log(response);
            userStore.updateState('role', response.role);
            console.log(response.role);
            return postToServer({ url: `http://smart.agromelio.ru:8080/api/profiles/${response.role}s/me`, request: 'get' })
          })
          .then((response) => {
            console.log(response);
